...
export const DataProvider = ({ children }: { children: ReactNode }) => {
    ...
    const addTransaction = async (transaction: Omit<Transaction, 'id'>) => {
        ...
        const newTransRef = await addDocToCollection('transactions', transactionToSave);
        const newTransaction = { id: newTransRef.id, ...transaction } as Transaction;
        setAllTransactions(prev => [newTransaction, ...prev].sort((a,b) => (b.date as Date).getTime() - (a.date as Date).getTime()));

        // Perform accounting updates locally after DB write
        if (transaction.type === 'income') {
            setBankAccounts(prev => prev.map(acc => acc.id === transaction.accountId ? { ...acc, balance: acc.balance + transaction.amount } : acc));
        } else if (transaction.type === 'expense') {
            if (transaction.cardId) { // It's a card payment
                const card = bankCards.find(c => c.id === transaction.cardId);
                if (card?.cardType === 'credit') {
                    setBankCards(prev => prev.map(c => c.id === transaction.cardId ? { ...c, usedAmount: (c.usedAmount || 0) + transaction.amount } : c));
                } else { // Debit or Prepaid card
                     setBankAccounts(prev => prev.map(acc => acc.id === transaction.accountId ? { ...acc, balance: acc.balance - transaction.amount } : acc));
                }
            } else if (transaction.isCreditLinePayment) { // It's a credit line payment from the bank account itself
                 setBankAccounts(prev => prev.map(acc => acc.id === transaction.accountId ? { ...acc, creditLineUsed: (acc.creditLineUsed || 0) + transaction.amount } : acc));
            ...
            setBankAccounts(prev => prev.map(acc => {
                ...
                if (acc.id === transaction.destinationAccountId) return { ...acc, balance: acc.balance + transaction.amount };
                return acc;
            }));
        }
    };
    ...
    const deleteTransaction = async (id: string) => {
        // This is also complex. Deleting a transaction should revert its accounting effects.
        // This is best handled by a Cloud Function onDelete. For now, we just delete the record.
        await deleteDocFromCollection('transactions', id);
        setAllTransactions(prev => prev.filter(t => t.id !== id));
        // A full refetch might be needed to ensure consistency if onDelete function is not used
        if(uid){
             const updatedAccounts = await fetchData('bankAccounts', uid);
            setBankAccounts(updatedAccounts as BankAccount[]);
            const updatedCards = await fetchData('bankCards', uid);
            setBankCards(updatedCards as BankCard[]);
        }
    };
    ...
    const addDebtPayment = async (payment: Omit<DebtPayment, 'id'>) => {
        ...
        const savedPayment = await addDocToCollection('debtPayments', { ...payment, date: Timestamp.fromDate(payment.date) });
    ...
    const addGoalContribution = async (contribution: Omit<GoalContribution, 'id'>) => {
        ...
        const contributionRef = await addDocToCollection('goalContributions', { ...contribution, date: Timestamp.fromDate(contribution.date) });
        ...
        setGoalContributions(prev => [...prev, { ...contributionRef } as GoalContribution]);
    ...
    const addInvestmentContribution = async (contribution: Omit<InvestmentContribution, 'id'>) => {
        ...
        const contributionRef = await addDocToCollection('investmentContributions', dataToSave);
        ...
        setInvestmentContributions(prev => [...prev, { ...contributionRef } as InvestmentContribution]);
    ...
    const addTaxPayment = async (payment: Omit<TaxPayment, 'id'>) => {
        ...
        const paymentRef = await addDocToCollection('taxPayments', finalPayment);
        ...
        setTaxPayments(prev => [...prev, { ...paymentRef } as TaxPayment]);
    };
